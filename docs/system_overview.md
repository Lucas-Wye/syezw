...
- Upload/download rate limit: at least 30 seconds between two successful attempts (client side). If
  the last attempt failed, the next attempt is not throttled.
...
Room is the **source of truth** on the device. The backend stores encrypted payloads and image
blobs.
...
- `POST /sync/download`
    - Accepts client metadata and returns only server records that are missing or outdated on the
      client.
...
- Backend returns only records that are missing or outdated on `/sync/download` (metadata-based
  incremental sync).
